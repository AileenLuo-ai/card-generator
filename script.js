...
document.getElementById('opt-font-hand').addEventListener('change', (e) => {
  document.querySelectorAll('.handwritten-input, .side-fields input').forEach(el => {
...
const spacingLabels = { '0.5': 'Tight', '0.6': 'Tight', '0.7': 'Compact', '0.8': 'Compact', '0.9': 'Snug', '1': 'Normal', '1.1': 'Relaxed', '1.2': 'Relaxed', '1.3': 'Roomy', '1.4': 'Roomy', '1.5': 'Spacious' };
...
const fontSizeLabels = { '0.8': 'Small', '0.85': 'Small', '0.9': 'Compact', '0.95': 'Compact', '1': 'Normal', '1.05': 'Medium', '1.1': 'Medium', '1.15': 'Large', '1.2': 'Large', '1.25': 'XL', '1.3': 'XL' };
...
function createPrintCard(fontBody, fontHand, isVintage) {
  ...
  const qualities = ['look', 'wit', 'outfit', 'height', 'laugh', 'energy', 'smile', 'kindness', 'friend', 'presence'];
  const adjectives = ['is hard to ignore.', 'is intriguing.', 'caught my eye.'];
  const whens = ['soon.', 'one day.'];
  const contacts = ['Phone', '@', 'Email'];

  const bub = (text) =>
    `<span class="pc-bubble">${text}</span>`;
  const blank = (width) =>
